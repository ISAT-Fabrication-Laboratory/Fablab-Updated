...
class AdminChat extends BaseController
{
    ...
    {
        return view('dashboard.admin.adminchat');
    }
    public function messages()
    {
        ...
        foreach ($users as $user) {
            $message = $this->chat
                ...
                ->where('admin_id', $this->session->get('id'))
                ->latest()
                ->first();
            if ($message) {
                if ($message->type == "text") {
                    ...
                    <a href="' . url("/chatnow/{$user->id}") . '" class="name-message text-white m-0 text-decoration-none" id="chatnow">' . $user->name . '</a>
                    ';
    ...
    {
        $users = $this->users->where('type', '0')->get();
        foreach ($users as $user) :
    ...
    public function chatnow($id)
    {
        ...
        return view('dashboard.admin.chatnow')->with('user_id', $id)->with('usersname', $user_name)->with('userrole', $user_role)->with('userpicture', $user_picture);
        $this->userchatshow($id);
    }

    public function userchatshow($id)
    {
        $chats = $this->chat->where('user_id', $id)->where('admin_id', $this->session->get('id'))->get();
        $user = $this->users->where('id', $id)->first();
        ...
        foreach ($chats as $chat) :
            if ($chat->type === 'text') {
                if ($chat->sender === $this->session->get('id')) {
                    ...
                    $profiles = asset('storage/userpictures/' . $user->picture);
                ...
                if ($chat->sender === $this->session->get('id')) {
                    ...
                    $profile_pic = asset('storage/userpictures/' .  $user->picture);
                    $imgdir = asset('storage/chatimages/' .  $chat->data);
    ...
    public function sendmessage(Request $request)
    {
        $this->chat->create([
            ...
            'admin_id' => $request->input('sender_id'),
